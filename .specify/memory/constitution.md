...
- **Azureの機密情報（サブスクリプションID、テナントID、クライアントID）をコード・ログ・CI/CD出力・PRコメント・Step Summary等の公開可能な場所に平文で露出させないこと**
- CI/CDワークフローでは `::add-mask::` によるランタイムマスクと `sed` 等によるテキスト置換の二重防御を必須とする
